...
import top.kagg886.pmf.i18n.home_recommend_illust
import top.kagg886.pmf.i18n.home_recommend_novel
...
@Serializable
data object RecommendRoute : SerializableNavKey

@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun RecommendScreen(content: @Composable () -> Unit) {
    val model = viewModel<RecommendModel> { RecommendModel() }
    val state by model.collectAsState()
    val nav = LocalNavController.current

    model.collectSideEffect { effect ->
        when (effect) {
            is RecommendModelEffect.NavigateTo -> nav.update {
                replaceTop(effect.destination)
            }
        }
    }

    TraceEffect("RecommendScreen", state.selectedTab) {
        i("Recommend sub-tab changed (tab=${state.selectedTab})")
    }
            ...
            RecommendTab.entries.forEach { tab ->
                Tab(
                    ...
                    text = {
                        Text(
                            text = when (tab) {
                                RecommendTab.ILLUST -> stringResource(Lang.string.home_recommend_illust)
                                RecommendTab.NOVEL -> stringResource(Lang.string.home_recommend_novel)
                            },
                        )
                    },
                )
